.. _forking:

======================================================
Making your own copy (fork) of rpe
======================================================

You need to do this only once.  The instructions here are very similar
to the instructions at http://help.github.com/forking/ |emdash| please see
that page for more detail.  We're repeating some of it here just to give the
specifics for the `rpe`_ project, and to suggest some default names.

Set up and configure a github account
=====================================

If you don't have a github account, go to the github page, and make one.

You then need to configure your account to allow write access |emdash| see
the ``Generating SSH keys`` help on `github help`_.

Create your own forked copy of `rpe`_
======================================================

#. Log into your github account.
#. Go to the `rpe`_ github home at `rpe github`_.
#. Click on the *fork* button:

   .. image:: forking_button.png

   Now, after a short pause and some 'Hardcore forking action', you
   should find yourself at the home page for your own forked copy of `rpe`_.

.. include:: links.inc

